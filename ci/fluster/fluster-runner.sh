...
if [ -z "$CPU_VERSION" ]; then
   echo 'CPU_VERSION must be set to something like "radeonsi-raven" or "freedreno-a630" (the name used in your ci/gpu-version-*.txt)'
   exit 1
fi
...
# TODO: Remove when not debugging anymore
/opt/fluster/fluster.py list --check --verbose
gst-inspect-1.0
...
for codec in ${FLUSTER_CODECS}; do
    DECODERS="$DECODERS GStreamer-${codec}-V4L2SL-Gst1.0"
done

# Default to an empty known flakes file if it doesn't exist.
FLUSTER_FLAKES=$INSTALL/$CPU_VERSION-fluster-flakes.txt
touch ${FLUSTER_FLAKES}

# Default to an empty known fails file if it doesn't exist.
FLUSTER_FAILS=$INSTALL/$CPU_VERSION-fluster-fails.txt
touch ${FLUSTER_FAILS}

# Default to an empty known skips file if it doesn't exist.
FLUSTER_SKIPS=$INSTALL/$CPU_VERSION-fluster-skips.txt
touch ${FLUSTER_SKIPS}
