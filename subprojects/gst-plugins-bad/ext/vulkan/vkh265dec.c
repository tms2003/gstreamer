...
gst_vulkan_h265_decoder_open (GstVideoDecoder * decoder)
{
  ...
  if (!gst_vulkan_device_run_context_query (GST_ELEMENT (decoder),
    ...
    self->device =
        gst_vulkan_device_new_with_index (self->instance, klass->device_id);
    if (!self->device || !gst_vulkan_device_open (self->device, &error)) {
      GST_ELEMENT_ERROR (self, RESOURCE, NOT_FOUND,
...
gst_vulkan_h265_decoder_class_init (gpointer g_klass, gpointer class_data)
{
  ...
  vk_h265_class->device_id = cdata->device_id;
    ...
    guint rank)
{
  ...
  cdata->device_id = device->physical_device->device_index;
