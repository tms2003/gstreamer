...
#include <math.h>
#include <gst/codecparsers/gstvp8bitwriter.h>
    ...
    gboolean bump_all, GstVideoCodecFrame ** out_frame)
{
  ...
  /* We don't reorder - if there is no new frame there will be no new
     output frame. */
  if (frame == NULL)
    goto out;
...
gst_va_vp8_enc_reconfig (GstVaBaseEnc * base)
{
  ...
  guint max_surfaces = 0, codedbuf_size;
  gint width, height;
  ...
  if (!gst_va_encoder_is_open (base->encoder)
      && !gst_va_encoder_open (base->encoder, base->profile,
          ...
          GST_VP8_MAX_REF_FRAMES, self->rc.rc_ctrl_mode, 0)) {
    ...
    VAEncPictureParameterBufferVP8 * pic_param)
{
  ...
  *pic_param = (VAEncPictureParameterBufferVP8) {
    ...
    .ref_flags.bits = {
      ...
      .no_ref_last = 1, /* maybe changed later */
      .no_ref_gf = 1,
      .no_ref_arf = 1,
      .temporal_id = 0,
      .first_ref = 0,
  ...
  if (va_frame->type == GST_VP8_INTER_FRAME) {
    ...
    pic_param->ref_last_frame = gst_va_encode_picture_get_reconstruct_surface
        (_enc_frame (self->gop.last_ref)->picture);
    ...
    GstVideoCodecFrame * gst_frame, gboolean is_last)
{
  ...
  if (va_frame->frame_num == 0) {
    ...
    if (!_vp8_add_sequence_param (self, va_frame->picture, &seq_param))
      return GST_FLOW_ERROR;
  }
    ...
    GstVideoCodecFrame * frame, gboolean * complete)
{
  ...
  buf =
      gst_va_base_enc_create_output_buffer (base, frame_enc->picture, NULL, 0);
  if (!buf) {
