...
gst_v4l2_codec_vp9_dec_negotiate (GstVideoDecoder * decoder)
{
  ...
  if (!self->streaming) {
    ...
    self->output_state =
        ...
        &self->vinfo, &self->vinfo_drm, self->width, self->height,
        vp9dec->input_state);
...
gst_v4l2_codec_vp9_dec_check_resolution_change (GstV4l2CodecVp9Dec * self,
    ...
    GstVideoCodecFrame * frame, GstVp9Picture * picture)
{
  ...
  return gst_v4l2_codec_vp9_dec_check_resolution_change (self, picture);
}
    ...
    GstVideoCodecFrame * frame, GstVp9Picture * picture)
{
  ...
  if (gst_v4l2_codec_vp9_dec_check_resolution_change (GST_V4L2_CODEC_VP9_DEC
          (decoder), picture) != GST_FLOW_OK) {
    return NULL;
  }
    ...
    GstVideoCodecFrame * frame, GstVp9Picture * picture)
{
  ...
  vinfo = gst_video_info_new_from_caps (self->output_state->caps);
  if (GST_VIDEO_INFO_WIDTH (vinfo) != frame_hdr->width ||
      GST_VIDEO_INFO_HEIGHT (vinfo) != frame_hdr->height) {
    if (self->output_state)
      gst_video_codec_state_unref (self->output_state);
