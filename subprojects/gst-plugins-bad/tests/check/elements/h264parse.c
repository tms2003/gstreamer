...
GST_START_TEST (test_insert_cc_a53_sei)
{
  ...
  video_buf = gst_buffer_new_and_alloc (video_idr_size);
  gst_buffer_add_video_caption_meta (video_buf,
      GST_VIDEO_CAPTION_TYPE_CEA708_RAW, cc_data, cc_data_size);
  gst_buffer_fill (video_buf, 0, video_idr, video_idr_size);
  fail_unless_equals_int (gst_harness_push (h, video_buf), GST_FLOW_OK);

  video_buf = gst_harness_pull (h);
  fail_unless_equals_int (gst_buffer_get_size (video_buf),
      caption_sei_size + video_idr_size);
  fail_unless_equals_int (gst_buffer_memcmp (video_buf, 0, caption_sei,
          caption_sei_size), 0);
  fail_unless_equals_int (gst_buffer_memcmp (video_buf, caption_sei_size,
          video_idr, video_idr_size), 0);
