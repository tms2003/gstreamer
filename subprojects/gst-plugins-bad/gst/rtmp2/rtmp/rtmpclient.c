...
parse_conn_token (gchar type, const gchar * value, GError ** error)
{
  ...
  switch (type) {
    ...
    case 'B':
      ...
      gboolean bool_ = g_ascii_strtoull (value, &end_ptr, 10);
      if (end_ptr[0] != '\0') {
...
static gboolean
parse_librtmp_style_conn_props (const gchar * connect_string, GPtrArray * array, GError ** error)
{
  g_return_val_if_fail (error == NULL || *error == NULL, FALSE);
...
send_connect (GTask * task)
{
  ...
  if (data->location.extra_connect_args
    ...
    gboolean conn_result =
        parse_librtmp_style_conn_props (data->location.extra_connect_args, arguments,
        &error);
  ...
  gst_rtmp_connection_send_command_with_args (data->connection, send_connect_done,
      task, 0, "connect", arguments->len, (const GstAmfNode **) arguments->pdata);
