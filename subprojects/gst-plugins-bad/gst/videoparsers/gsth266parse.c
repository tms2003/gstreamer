...
G_DEFINE_TYPE (GstH266Parse, gst_h266_parse, GST_TYPE_BASE_PARSE);
...
{
  GST_DEBUG_OBJECT (h266parse, "reset frame");
...
static void
gst_h266_parse_format_from_caps (GstCaps * caps, guint * format, guint * align)
{
    ...
    GstCaps * in_caps)
{
  ...
  if (in_caps && caps) {
    if (gst_caps_can_intersect (in_caps, caps)) {
      ...
      gst_h266_parse_format_from_caps (in_caps, &format, &align);
      gst_caps_unref (caps);
      caps = NULL;
    }
  }
  ...
  if (caps && !gst_caps_is_empty (caps)) {
    ...
    gst_h266_parse_format_from_caps (caps, &format, &align);
  }
    ...
    guint size)
{
  ...
  GST_DEBUG_OBJECT (h266parse, "nal length %d", size);
...
gst_h266_parse_store_nal (GstH266Parse * h266parse, guint id,
    GstH266NalUnitType naltype, GstH266NalUnit * nalu)
{
  GstBuffer *buf, **store;
...
update_idr_pos (GstH266Parse * h266parse, GstH266NalUnit * nalu)
{
  ...
  GST_DEBUG_OBJECT (h266parse, "find %s in frame at offset %d, "
...
gst_h266_parse_process_nal (GstH266Parse * h266parse, GstH266NalUnit * nalu)
{
  ...
  GST_DEBUG_OBJECT (h266parse, "processing nal of type %u %s, size %u",
  ...
  switch (nal_type) {
    case GST_H266_NAL_VPS:
      ...
      gst_h266_parse_store_nal (h266parse, vps.vps_id, nal_type, nalu);
    ...
    case GST_H266_NAL_SPS:
      ...
      pres = gst_h266_parser_parse_sps (nalparser, nalu, &sps, TRUE);

      /* arranged for a fallback sps.sps_id, so use that one and only warn */
      if (pres != GST_H266_PARSER_OK) {
        /* try to not parse VUI */
        pres = gst_h266_parser_parse_sps (nalparser, nalu, &sps, FALSE);
        if (pres != GST_H266_PARSER_OK) {
      ...
      gst_h266_parse_store_nal (h266parse, sps.sps_id, nal_type, nalu);
      h266parse->header = TRUE;
      h266parse->state |= GST_H266_PARSE_STATE_GOT_SPS;
      break;
    case GST_H266_NAL_PPS:
      ...
      gst_h266_parse_store_nal (h266parse, pps.pps_id, nal_type, nalu);
      h266parse->header = TRUE;
      h266parse->state |= GST_H266_PARSE_STATE_GOT_PPS;
      break;
    ...
    case GST_H266_NAL_SLICE_GDR:
    {
      ...
      GST_DEBUG_OBJECT (h266parse, "parse result %d, "
    ...
    GstBaseParseFrame * frame, gint * skipsize)
{
  ...
  if (drain && current_off == size) {
    GST_DEBUG_OBJECT (h266parse, "draining with no new data");
    nalu.size = 0;
  ...
  GST_DEBUG_OBJECT (h266parse, "last parse position %d", current_off);
  ...
  while (TRUE) {
    ...
    switch (pres) {
      case GST_H266_PARSER_OK:
        GST_DEBUG_OBJECT (h266parse, "complete nal (offset, size): (%u, %u)",
      ...
      broken:
        ...
        if (current_off == 0) {
          ...
          *skipsize = nalu.offset;
          ...
          GST_DEBUG_OBJECT (h266parse, "terminating au");
          nalu.size = 0;
    ...
    GST_DEBUG_OBJECT (h266parse, "%p complete nal found. Off: %u, Size: %u",
    ...
    GST_DEBUG_OBJECT (h266parse, "Looking for more");
    current_off = nalu.offset + nalu.size;
...
skip:
  GST_DEBUG_OBJECT (h266parse, "skipping %d", *skipsize);
...
gst_h266_parse_make_codec_data (GstH266Parse * h266parse)
{
  ...
  GST_WARNING ("Codec data is not supported now.");
    ...
    GstBuffer * buffer, GstBaseParseFrame * frame)
{
  ...
  if (parse->align == GST_H266_PARSE_ALIGN_NAL) {
    ...
    GST_DEBUG_OBJECT (parse, "- sending VPS/SPS/PPS");
