...
session_rpsi (const gchar * key, RTPSource * source, ReportData * data)
{
  ...
  msg_len = GST_ROUND_UP_4 (2 + sess->rpsi_payload_lenght);
  /* padding in bits */
  pb = (msg_len - 2 - sess->rpsi_payload_lenght) * 8;
  ...
  for (i = 0; i < sess->rpsi_payload_lenght && i < 32; i++) {
...
gboolean
rtp_session_rpsi (RTPSession * sess, guint8 pt, guint8 * data, guint lenght)
{
  gint i;
  ...
  sess->rpsi_pt = pt;
  sess->rpsi_payload_lenght = lenght;
  for (i = 0; i < sess->rpsi_payload_lenght && i < 32; i++)
    sess->rpsi_payload[i] = data[i];
