...
gst_ffmpeg_parser_send_updated_src_caps (GstFFMpegParser *parser)
{
  ...
  if (src_caps == NULL) {
    GST_ELEMENT_ERROR (parser, STREAM, WRONG_TYPE,
        ("gst_ffmpeg_codecid_to_caps(): failed to deduce CAPS from codec "
            "id and context"), NULL);
    return NULL;
  }
  ...
  if (!gst_pad_set_caps (GST_BASE_PARSE_SRC_PAD (parser), src_caps)) {
    ...
    GST_ELEMENT_ERROR (parser, STREAM, WRONG_TYPE,
        ("gst_pad_set_caps(): failed to set 'src' pad CAPS to %s", caps_str),
        NULL);
    g_free (caps_str);
...
gst_ffmpeg_parser_register (GstPlugin *plugin)
{
  ...
  for (;;) {
    ...
    for (size_t i = 0; i < (sizeof (parser->codec_ids) / sizeof (int)); ++i) {
      ...
      gboolean result =
          gst_element_register (plugin, type_name, GST_RANK_PRIMARY, type);
      if (!result) {
